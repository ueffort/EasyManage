...
(function ($)
{
    ...
    window.liger = $.ligerui = {
        ...
        add: function (manager)
        {
			...
			if (typeof(manager.element) == 'object') $(manager.element).attr(this.idAttrName,manager.id);
        ...
        remove: function (arg)
        {
            if (typeof arg == "string" || typeof arg == "number")
            {
				...
				//delete $.ligerui.managers[arg];
					...
					delete $.ligerui.managers[$(arg).attr(this.idAttrName)];
            }
            else if (typeof arg == "object")
            {
                if(arg instanceof $.ligerui.core.Component)
				{
        ...
        {
            idAttrName = idAttrName || this.idAttrName;
            if (typeof arg == "string" || typeof arg == "number")
            {
				//通过jQuery找寻被设定的ID号
                //return $.ligerui.managers[arg];
					if(!$(arg).attr(idAttrName)) return null;
					return $.ligerui.managers[$(arg).attr(idAttrName)];
            }
            else if (typeof arg == "object")
            {
                if(arg instanceof $.ligerui.core.Component)
				...
				{
					if (!$(arg).attr(this.idAttrName)) return false;
					return $.ligerui.managers[$(arg).attr(this.idAttrName)];
				}
            }
            return null;
        },
	...
	/*$.fn.removeligerui = $.fn.remove;
	$.fn.remove = function(){
		$.each($('[ligeruiid]',this),function(k,i){
			$.ligerui.remove(i);
		});
		$("*", this).each(function(){
			$.event.remove(this);
		...
		this.html();
		this.removeligerui.apply(this,arguments);
	}*/
    ...
    $.ligerui.core.UIComponent.ligerExtend($.ligerui.core.Component, {
        ...
        _init: function ()
        {
            ...
            //}
			//	this.id = $.ligerui.getId(this.__idPrev());
			//全部ID号自生成end
            //存入管理器池
            $.ligerui.add(this);
...
(function ($)
{
    ...
    $.ligerDefaults.Button = {text: 'Button', disabled: false };
    ...
    $.ligerui.controls.Button.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            g.button.append('<div class="l-btn-l"></div><div class="l-btn-r"></div><div class="l-btn-inner"></div>');
            p.click && g.button.click(function ()
            {
                if (!p.disabled)
                    p.click();
            });
			g.button.live('mouseover', function ()
			{
        ...
        {
            $("div.l-btn-inner", this.button).html(value);
        },
        setValue: function (value)
        {
            this.set('text', value);
...
(function ($)
{
    ...
    $.ligerDefaults.DateEditor = {
        ...
        onChangeDate: false,
        absolute: true,  //选择框是否在附加到body,并绝对定位
		thirdparty:false //是否使用第三方控件，如：My97DatePicker
    };
	//第三方扩展扩展
    $.ligerDefaults.DateEditor.thirdpartys = $.ligerDefaults.DateEditor.thirdpartys || {};
	$.ligerDefaults.DateEditor.thirdpartys['My97DatePicker'] = {
		setValue:function(value){
			var g = this, p = this.options;
			g.inputText.val(value);
			g.inputText.change(function(){
				g.trigger('changeDate',[value]);
				g.trigger('validate', [value]);
			});
			g.trigger('validate', [value]);
		},
		getValue:function(){
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (g.inputText.val() != "")
                g.onTextChange();
			if(p.thirdparty && typeof(p.thirdpartys[p.thirdparty]) == 'object'){
				g.thirdparty = p.thirdpartys[p.thirdparty];
        ...
        showDate: function ()
        {
            ...
            this.inputText.trigger("changeDate",[dateStr]).focus();
			this.inputText.trigger("validate",[dateStr]);
        },
        ...
        _setValue: function (value)
        {
            var g = this;
			if(g.thirdparty){
				g.thirdparty.setValue.call(g,value);
			}else{
				...
				else if (typeof value == "object")
				{
					...
					{
						var value = g.getFormatDate(value)
						g.inputText.val(value);
					}
				}
				g.onTextChange();
        ...
        {
			var g = this;
			if(g.thirdparty) return g.thirdparty.getValue.call(g);
            return g.inputText.val();
        },
        setEnabled: function ()
        {
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
		...
		onBeforeClose:false,   //关闭前事件
		onAfterClose:false,    //关闭后事件
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
					...
					g.jiframe.load(function(){
						var framename = g.jiframe.attr('name');
						...
						if(frm != null && subWeb != null){
							var contentHeight = subWeb.documentElement.scrollHeight;
							g.jiframe.height(subWeb.documentElement.scrollHeight);
						}
						g.initLT();
					});
					...
					if(p.data){
						...
						.attr('action',p.url);
						for(var i in p.data){
							form.append("<input type='hidden' name='"+i+"' value='"+p.data[i]+"' />");
						}
						form.appendTo(g.dialog.content);
						//插入HTML文档后才能提交，不然会打开新窗口
						setTimeout(function ()
						{
							form[0].submit();
						}, 0);
            ...
            {
				//close even
				if (g.hasBind('beforeClose') && g.trigger('beforeClose') == false) return false;
                if (p.isHidden)
                    g.hide();
                else
                    g.close();
				if (g.hasBind('afterClose') && g.trigger('afterClose') == false) return false;
            });
			g.initLT();
            g.show();
		...
		reload: function(){
			this.setUrl(this.options.url,this.options.data);
		},
...
(function ($)
{
    ...
    l.controls.Drag.ligerExtend(l.core.UIComponent, {
        ...
        _start: function (e)
        {
            ...
            g.handler.bind("selectstart.drag", function () { return false; });
            g.handler.bind('mousemove.drag', function ()
            {
                g._drag.apply(g, arguments);
            });
            l.draggable.dragging = true;
            g.handler.bind('mouseup.drag', function ()
            {
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
		...
		actionParmName: 'dataaction',			//服务器加载提交参数，用于判断页面性质(提交给服务器)
	...
	$.ligerDefaults.Grid.formatters['select'] = function (value, column)
    {
		var split = column.split || ',';
		var valueField = column.valueField || 'id';
        var textField = column.textField || 'text';
		var values = [];
		...
		for(var i in values){
			for(var j in column.select){
				if(column.select[j][valueField] == values[i]) values[i] = column.select[j][textField];
			}
		}
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
            var g = this, p = this.options;
			if($(g.element).is(':input')){
				g.input = true;
				g.grid = $('<div></div>');
				$(g.element).after(g.grid);
			}else{
				g.grid = $(g.element);
			}
        ...
        _setHeight: function (h)
        {
            ...
            h -= gridHeaderHeight;
			if(g.grid.children().not('.l-panel-bwarp,.l-grid-loading').not(':hidden').each(function(){
        ...
        {
            this.loadData(this.options.rowsdata);
        },
        //刷新数据
        loadData: function (loadDataParm)
        {
            ...
            if (p.data)
            {
                ...
                else if (typeof p.data == "object")
                {
                    ...
                    {
                        param.push({ name: name, value: p.data[name] });
                    }
                }
            }
            if (p.dataAction == "server")
            {
				...
				param.push({ name: p.actionParmName, value: p.dataAction });
            };
            $(".l-bar-btnload span", g.toolbar).addClass("l-disabled");
            if (p.dataType == "local")
            {
                g.filteredData = g.rowsdata = p.rowsdata;
                if (clause)
            ...
            {
                if (g.rowsdata && g.rowsdata[p.root])
                {
                    g.filteredData = g.rowsdata;
                    if (clause)
                        g.filteredData[p.root] = g._searchData(g.filteredData[p.root], clause);
                    g.currentData = g._getCurrentPageData(g.filteredData);
                    g._showData();
        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    {
                        g.currentData = g.rowsdata = {};
                        g.currentData[p.root] = g.rowsdata[p.root] = [];
                        g.currentData[p.record] = g.rowsdata[p.record] = 0;
                        g._showData();
                        return;
                    }
                    g.rowsdata = data;
        ...
        */
        ...
        _showData: function ()
        {
            ...
            {
                //更新总记录数
                if (p.dataAction == "server" && g.rowsdata && g.rowsdata[p.record])
                    p.total = g.rowsdata[p.record];
                else if (g.filteredData && g.filteredData[p.root])
                    p.total = g.filteredData[p.root].length;
                else if (g.rowsdata && g.rowsdata[p.root])
                    p.total = g.rowsdata[p.root].length;
                else if (data)
                    p.total = data.length;
            ...
            if (p.totalRender)
            {
                ...
                $(".l-panel-bar", g.element).before('<div class="l-panel-bar-total">' + p.totalRender(g.rowsdata, g.filteredData) + '</div>');
            }
        ...
        _getCellHtml: function (rowdata, column)
        {
			...
			if(column.tip) htmlarr.push(' title="'+content+'"');
        ...
        _onContextmenu: function (e)
        {
            ...
            if (src.row)
            {
                ...
                {
                    return g.trigger('contextmenu', [src.data, src.data['__index'], src.row,e]);
                }
            }
            else if (src.hcell)
            {
        ...
        _onDblClick: function (e)
        {
            ...
            if (src.row)
            {
                g.trigger('dblClickRow', [src.data, src.data['__id'], src.row,e]);
            }
        },
        _onClick: function (e)
        {
        ...
        select: function (rowParm)
        {
            ...
            if (!g.enabledCheckbox() && !ctrlKey) //单选
            {
        ...
        {
            var g = this, p = this.options;
			if(g.grid.is(':hidden')) return false;
            if (p.height && p.height != 'auto')
            {
                ...
                if (typeof (p.height) == "string" && p.height.indexOf('%') > 0)
                {
                    ...
                    if (p.InWindow || (gridparent[0] && gridparent[0].tagName.toLowerCase() == "body")){
	...
	var resizeGrid = function (e){
		var arr = $.ligerui.find('$.ligerui.controls.Grid');
		if(!arr.length) return ;
		for(var i in arr){
			var item = arr[i];
			item._onResize.call(item);
		}
    }
	var keydownGrid = function (e){
		if (e.ctrlKey) ctrlKey = true;
	}
	var keyupGrid = function(e){
		if (e.ctrlKey) ctrlKey = false;
	}
	$(window).bind('resize.grid',resizeGrid);
	$(window).bind('keydown.grid',keydownGrid);
	$(window).bind('keyup.grid',keyupGrid);
...
(function ($)
{
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.menu.bind('click.menu', function ()
            {
        ...
        addItem: function (item, menu)
        {
            ...
            if(!item.id) item.id = 'item-'+g.menuItemCount;
            ...
            item.color && ditem.prepend('<div class="l-menu-item-color" style="background:'+item.color+';"></div>');
        ...
        createMenu: function (parentMenuItemID)
        {
			...
			menu.hover(function()
			{
				if(++g.overCount<=0) g.overCount = 1;
			},function ()
            {
				if(--g.overCount <=0 && !g.menu.showedSubMenu) g.hide();
            });
			menu.showedSubMenu = false;
            if (parentMenuItemID)
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
		...
		//如果当前ID有父TAB，则选中父TAB，并关闭该ID
		removeChildSelectedFather: function(tabid)
		{
			var g = this, p = this.options;
			var fatherid = $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).attr('fatherid');
			if(fatherid){
				g.selectTabItem(fatherid);
				g.removeTabItem(tabid);
				return true;
			}else{
				return false;
        ...
        overrideTabItem: function (targettabid, options)
        {
            ...
            if (g.trigger('beforeOverrideTabItem', [targettabid]) == false)
                return false;
			g.removeTabItem(targettabid);
            g.addTabItem(options);
			/*
			//表单提交会打开新页面
        ...
        addTabItem: function (options)
        {
            ...
            tabitem.attr("tabid", tabid);
            contentitem.attr("tabid", tabid);
			if(fatherid) contentitem.attr("fatherid",fatherid);
            if (url)
            {
                iframe.attr("name", tabid)
				 ...
				 if(options.data){
					...
					for(var i in options.data){
						form.append("<input type='hidden' name='"+i+"' value='"+options.data[i]+"' />");
					}
        ...
        reload: function (tabid)
        {
            ...
            var iframeloading = $(".l-tab-loading:first", contentitem);
            var iframe = $("iframe:first", contentitem);
			var form = $("form:first",contentitem);
			if(form.length){
				form[0].submit();
			}else{
...
(function ($)
{
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
		...
		_setText: function (value)
        {
            ...
            {
                this.inputText.attr("readonly", "readonly");
                this.wrapper.addClass("l-text-text");
            }
            else
            {
                this.inputText.removeAttr("readonly");
                this.wrapper.removeClass('l-text-text');
            }
        },
        _setWidth: function (value)
        {
